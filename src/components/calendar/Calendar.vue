      ...
      <div class="wrapper">
        ...
        <table cellpadding="5" class="now-month">
          ...
          <tr v-for="(day,k1) in days" :key="k1" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :key="k2" :class="{'selected':child.selected,'disabled':child.disabled}" :style="{height: tdH + 'px'}" @click="select(k1,k2,$event)">
              <span>{{child.day}}</span>
              <span class="dot" v-if="child.eventName"></span>
            </td>
          </tr>
  ...
  export default {
    ...
    methods: {
      ...
      touchmove (e) {
        ...
        let k = this.endPos.y / this.endPos.x
        ...
        if (Math.abs(k) < 1) {
          ...
          e && e.preventDefault(); //阻止触摸事件的默认行为，即阻止滚屏
        }
      },
      touchend (e) {
...
<style scoped>
  ...
  /* .calendar > .calendar-box::-webkit-scrollbar { height: 0px !important;width: 0px !important; } */
