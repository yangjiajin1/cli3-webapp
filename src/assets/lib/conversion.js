/**
 * @file JS image compression tools
 ...
 */
const imagetoCanvas = async (image, config = {}) => {
 ...
 * @returns {Promise(string)} Promise含有一个dataURL字符串参数
 */
const canvastoDataURL = async (canvas, quality, type) => {
 ...
 */
const dataURLtoFile = async (dataURL, type) => {
 ...
 * @returns {Promise(Blob)}
 */
export const compressAccurately = async (file, config = {}) => {
  ...
  // 设置图片最大宽度
  if (config.widthMax && image.width > config.widthMax) {
    config.width = config.widthMax
  }
    ...
    if (x === 7) {
      if (resultSize.max < CalculationSize || resultSize.min > CalculationSize) {
        compressDataURL = [compressDataURL, ...tempDataURLs]
          ...
          .sort(
            (a, b) =>
              Math.abs(a.length * proportion - resultSize.accurate) - Math.abs(b.length * proportion - resultSize.accurate)
          )[0];
      }
      break;
    }
...
const dataURLToBlob = function (dataurl,fileName) {
  let arr = dataurl.split(","),
    ...
    theBlob.name = fileName || 'nameBlob';
    return theBlob;
}
